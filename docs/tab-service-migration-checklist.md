...
| `assignTabDetails` (index) | N/A (was always -1) | `tabService.getTabIndexInWindowProfile(tab)` | **NEW** - proper index |
| `notifyExtensionsOfChanges` | Never existed in old code | `Tab.notifyExtensionsOfChanges()` → `"tab-updated"` | **NEW** - emits on state change |
...
| Feature | Issue | Fix |
|---------|-------|-----|
| Extension state update on tab property changes | `notifyExtensionsOfChanges` was defined but never called | Added call in `wireTabEvents` `"updated"` handler |
